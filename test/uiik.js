...
class Uii {
    constructor(ele, opts) {
        ...
        this.opts = opts || {};
        if (isArrayLike$4(ele) && !isString$4(ele)) {
            this.ele = map$1(ele, (el) => {
                let e = isString$4(el) ? document.querySelector(el) : el;
                if (!isElement$1(e)) {
            ...
            const el = isString$4(ele) ? document.querySelectorAll(ele) : ele;
            if (!isElement$1(el) && !isArrayLike$4(el)) {
                console.error('Invalid element "' + ele + '"');
                return;
            }
            this.ele = isArrayLike$4(el) ? toArray$3(el) : [el];
        }
    }
    /**
    ...
    constructor(container, opts) {
        ...
        each$2(this.ele, con => {
            ...
            const minSizeAry = map$1(children, (c, i) => {
                if (isArray$4(this.opts.minSize)) {
...
class Resizable extends Uii {
    ...
    bindHandle(handle, dir, panel, opts) {
        ...
        this.addPointerDown(handle, ({ ev, onPointerStart, onPointerMove, onPointerEnd }) => {
            ...
            onPointerStart(function (args) {
                ...
                currentVertex = vertexBeforeTransform = map$1(originVertex, ({ x, y }) => {
            ...
            onPointerEnd((args) => {
                ...
                let centerX = x, centerY = y;
                const deg = matrixInfo.angle * ONE_ANG;
                const currentVertex = map$1(originVertex, ({ x, y }) => {
                    const nx = (x - centerX + sx) * Math.cos(deg) -
                        (y - centerY + sy) * Math.sin(deg);
                    const ny = (x - centerX + sx) * Math.sin(deg) +
                        (y - centerY + sy) * Math.cos(deg);
    ...
    initHandle(panel) {
        ...
        let handles;
        if (isString$4(handleStr)) {
            handles = document.querySelectorAll(handleStr);
        }
        else if (isFunction$4(handleStr)) {
        ...
        handles = isArrayLike$4(handles) ? handles : [handles];
        each$2(handles, (h) => {
            //get dir from handle
...
class Draggable extends Uii {
    constructor(els, opts) {
        ...
        if (this.opts.containment) {
            ...
            else if (isString$4(this.opts.containment)) {
    ...
    bindEvent(bindTarget, opts, handleMap) {
        ...
        this.addPointerDown(bindTarget, ({ ev, currentTarget, currentStyle, currentCStyle, pointX, pointY, onPointerStart, onPointerMove, onPointerEnd }) => {
            ...
            let findRs = find$1(draggableList, el => el.contains(t));
            if (!findRs)
                return;
            ...
            onPointerStart(function (args) {
                ...
                if (ghost) {
                    ...
                    if (ghostClass) {
                        copyNode.classList.add(...compact$1(split$1(ghostClass, ' ')));
                    }
                    copyNode.classList.add(...compact$1(split$1(classes, ' ')));
    ...
    onOptionChanged(opts) {
        ...
        if (!isFunction$4(droppable)) {
            if (isUndefined$3(droppable)) {
                opts.droppable = () => { };
            }
            else if (isString$4(droppable)) {
                opts.droppable = () => document.querySelectorAll(droppable);
            }
            else if (isArrayLike$4(droppable)) {
...
_Draggable_handleMap = new WeakMap(), _Draggable_container = new WeakMap(), _Draggable_instances = new WeakSet(), _Draggable_initStyle = function _Draggable_initStyle(draggableList) {
    each$2(draggableList, (el) => {
        ...
        if (isDefined$1(this.opts.cursor)) {
...
class CollisionDetector {
    constructor(el, targets, opts) {
        ...
        const domEl = isString$4(el) ? document.querySelector(el) : el;
        if (!domEl) {
    ...
    update() {
        let targets;
        if (isFunction$4(__classPrivateFieldGet(this, _CollisionDetector__targets, "f"))) {
            targets = __classPrivateFieldGet(this, _CollisionDetector__targets, "f").call(this);
        }
        else if (isString$4(__classPrivateFieldGet(this, _CollisionDetector__targets, "f"))) {
...
class Sortable extends Uii {
    ...
    active(draggingItem, fromContainer, toContainers, toOpts) {
        ...
        const acceptFn = isFunction$4(moveFrom) ? moveFrom : () => !!moveFrom;
        //验证移入策略
...
function bindContainer(registerEvent, container, opts) {
    registerEvent(container, "mousedown", (e) => {
        ...
        const toOutFn = isFunction$4(moveTo) ? moveTo : () => !!moveTo;
